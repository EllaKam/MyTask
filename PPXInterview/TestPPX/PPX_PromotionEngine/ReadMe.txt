...
Each one of the providers expose two methods :
	...
	1. GetItemDiscount - Gets the item and its original price and returns its discount.
	2. GetDiscountableItemIds - returns the ids of items that have discount (each call will return the same ids)
...
2. In case of failure/connection issue/any error from the provider = no dicount on the item.
